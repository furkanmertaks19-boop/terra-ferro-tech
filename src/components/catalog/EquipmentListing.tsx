...
export default function EquipmentListing({
  ...
  const base = pathFor("equipment", locale);
  ...
  return (
    <div className="bg-ivory text-ink">
      ...
      {groups.length > 0 && (
        <div className="border-y border-ink/10 bg-warm-white">
          <div className="container-site flex flex-wrap gap-x-5 gap-y-2 py-3">
            <GroupChip href={base} label={t.home.allOptions} active={!subcategory && !group} />
            {groups.map((item) => (
              <GroupChip
                key={item.id}
                href={`${base}?group=${item.id}`}
                label={item.label}
      ...
      <div className="mx-auto w-full max-w-[1400px] min-w-0 px-4 py-6 md:px-8 lg:py-10">
        <h2 className="sr-only">{t.productList.equipmentTitle}</h2>
        <p className="mb-3 text-sm text-ink/55">
          {products.length} {products.length === 1 ? t.productList.model : t.productList.models}
        </p>
        <div className="mb-5">
          <Suspense>
            <CatalogToolbar hideHp hidePrice searchPlaceholder={t.productList.searchEquipment} />
          </Suspense>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="border border-ink/10 bg-warm-white px-6 py-16 text-center">
            <p className="font-display text-2xl font-semibold">{t.productList.emptyTitle}</p>
          </div>
        )}
      </div>
    </div>
